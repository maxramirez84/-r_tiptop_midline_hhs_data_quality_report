...
api_token = api_token_moz # TIPTOP HHS Midline Mozambique
non_retrieved_records = c("cluster", "facility")
# File
file_prefix = "DATA/DATA_WITH_NO_DUPS/TIPTOPHHSMidlineMozambique"
file_content = "_DATA_WITH_NO_DUPS_"
file_date   ="2021-03-25"
file_time   ="16:38"

# Remove duplicated records
duplicated_records = T
...
sample_size             = 757
household_to_be_visited = 4650

data_timestamp = dataTimestamp(data_retrieval_mode, file_date, file_time)
```

---
title: "TIPTOP Midline HHS Data Quality Report: MOZ - NHAMATANDA"
...
if(!duplicated_records) {
  duplicated_records = duplicatedRecords(hhs_data, study_area_column, study_area_label)
  record_ids_to_remove = duplicated_records$record_id[
    duplicated(duplicated_records[2:ncol(duplicated_records)])]
  hhs_data = hhs_data[!(hhs_data$record_id %in% record_ids_to_remove), ]
}
...
Data collection by [CISM](http://manhica.org).

```{r recruited_women_area, echo=FALSE}
# Midline dataset adjustments
# ended_pregnancy variable was removed
hhs_data$diff_in_days = difftime(anytime(hhs_data$interview_date), 
                                 anytime(hhs_data$end_last_pregnancy), units = "days")
...
```{r interviewer_cluster, echo=FALSE}
#options(knitr.kable.NA = '')
#recordsByInterviewerPerCluster(hhs_data, study_area_column)
```