...
api_token = hhs_endline_nigeria # TIPTOP HHS Endline  Nigeria
non_retrieved_records = c("cluster", "facility")
# File
file_prefix = "TIPTOPHHSEndlineNigeria"
file_content = "_DATA_WITH_NO_DUPS_"
file_date   ="2021-09-22"
file_time   ="14:41"
...
`r language$progress.partner` [`r partner.name`](https://www.unical.edu.ng/).

```{r recruited_women_area, echo=FALSE}
# Midline dataset adjustments
# ended_pregnancy variable was removed
#changed date format due to error in formatting after exporting in csv.
hhs_data$interview_date <- as.POSIXct(hhs_data$interview_date, format = "%Y-%m-%d %H:%M:%S")
hhs_data$end_last_pregnancy <- as.POSIXct(hhs_data$end_last_pregnancy, format = "%Y-%m-%d")
